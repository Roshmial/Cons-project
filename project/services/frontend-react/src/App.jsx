...
function renderAssistantContractStrip(message, canSaveDashboard, onSaveDashboard) {
  ...
  if (['file_result', 'artifact_result', 'structured_result', 'job_result'].includes(resultKind)) return null;
  if (message?.meta?.recurring_summary) return null;
  if (!(canSaveDashboard && nextActions.includes('save_as_job'))) return null;
  return <div className="assistant-action-strip"><button className="ghost-btn ghost-btn-xs" type="button" onClick={() => onSaveDashboard(message)}>Сохранить как задачу</button></div>;
}
...
function MessageBubble({ message, bootstrap, onSaveDashboard, onOpenAttachment }) {
  ...
  const assistantContractStrip = renderAssistantContractStrip(message, canSaveDashboard, onSaveDashboard);
...
function ChatScreen({ appState, composerText, composerFiles, composerExistingFiles, requestPolicyDraft, sending, onComposerChange, onComposerKeyDown, onFileChange, onToggleExistingFile, onOpenUserFile, onDeleteFile, onApplyStarterPrompt, onRequestPolicyChange, onSend, onRenameThread, onArchiveThread, onDeleteThread, onDismissWelcome, onSaveDashboard, onOpenAttachment }) {
