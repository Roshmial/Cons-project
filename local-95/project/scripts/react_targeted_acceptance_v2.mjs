...
const chatTextUnique = `react-accept-chat-${stamp}`;
const userEmail = `react-accept-${stamp}@demo.local`;
const jobName = `react-accept-job-${stamp}`;
const jobAlias = `React accept alias ${stamp}`;
...
const chatMainText = await page.locator('main').innerText();
const threadList = await api('/threads?include_archived=1', {}, token);
let persistedInBackend = false;
for (const thread of (threadList.threads || []).slice(0, 12)) {
  const detail = await api(`/threads/${thread.id}`, {}, token);
  if ((detail.messages || []).some((m) => String(m.content || '').includes(chatTextUnique) || String(m.meta?.user_text || '').includes(chatTextUnique))) {
    persistedInBackend = true;
    break;
  }
}
results.chat = {
  visible_in_ui: chatMainText.includes(chatTextUnique),
  persisted_in_backend: persistedInBackend,
  file_visible_in_ui: chatMainText.includes('smoke-note.txt')
};
...
await page.getByLabel('Описание').fill('React acceptance job');
...
const adminOverviewText = await page.locator('main').innerText();
results.admin_overview = {
  screen_visible: adminOverviewText.includes('Административный обзор'),
  notice_controls_visible: adminOverviewText.includes('Объявление в чате')
};
...
await page.getByLabel('Имя').fill('React Accept User');
...
const createdUser = (usersApi.users || []).find((user) => user.email === userEmail);
let detailOk = false;
let historyOk = false;
if (createdUser) {
  const detail = await api(`/admin/users/${createdUser.id}`, {}, token);
  const history = await api(`/admin/users/${createdUser.id}/history`, {}, token);
  detailOk = detail.user?.email === userEmail;
  historyOk = Array.isArray(history.history) && history.history.length >= 1;
}
results.admin_users = {
  visible_in_ui: usersText.includes(userEmail),
  visible_in_backend: Boolean(createdUser),
  detail_ok: detailOk,
